# Utility-Expenditures-for-Cardiovascular-Disease-Medical-Expenditure-Panel-Survey-MEPS
...
############################
##1. create vectors for cost items
Family_Total   <-c("Family_RX",   "Family_DV",   "Family_OM",   "Family_IP",   "Family_ER",   "Family_OP",   "Family_OB",   "Family_HH")
Medicare_Total <-c("Medicare_RX", "Medicare_DV", "Medicare_OM", "Medicare_IP", "Medicare_ER", "Medicare_OP", "Medicare_OB", "Medicare_HH")
Medicaid_Total <-c("Medicaid_RX", "Medicaid_DV", "Medicaid_OM", "Medicaid_IP", "Medicaid_ER", "Medicaid_OP", "Medicaid_OB", "Medicaid_HH")
Private_Total  <-c("Private_RX",  "Private_DV",  "Private_OM",  "Private_IP",  "Private_ER",  "Private_OP",  "Private_OB",  "Private_HH")
AllPayers_Total<-c("Total_RX",    "Total_DV",    "Total_OM",    "Total_IP",    "Total_ER",    "Total_OP",    "Total_OB",    "Total_HH")

##2. calculate total cost per payer
Bind_Data$Family_Total    = rowSums(Bind_Data[, Family_Total],   na.rm =T)
Bind_Data$Medicare_Total  = rowSums(Bind_Data[, Medicare_Total], na.rm =T)
Bind_Data$Medicaid_Total  = rowSums(Bind_Data[, Medicaid_Total], na.rm =T)
Bind_Data$Private_Total   = rowSums(Bind_Data[, Private_Total],  na.rm =T)
Bind_Data$AllPayers_Total = rowSums(Bind_Data[, AllPayers_Total], na.rm =T)

###############################
## Create SF-6D from SF-12V2 ##
###############################
#Step 1: Recode NAs to SF-12V2 variables - Assign NA to -1 (Inapplicable) and -9 (Not Ascertained)
summary(Bind_Data[,c("ADDAYA42","ADPWLM42","ADMALS42","ADPAIN42","ADNRGY42","ADDOWN42","ADSOCA42")])

##1. ADDAYA42: HLTH LIMITS MOD ACTIVITIES SF-12V2. 
Bind_Data$ADDAYA42[Bind_Data$ADDAYA42 < 0] <- NA
##2. ADPWLM42:  4WKS:WORK LIMT B/C PHY PROBS SF-12V2.
Bind_Data$ADPWLM42[Bind_Data$ADPWLM42 < 0] <-NA
##3. ADMALS42: 4WKS:ACCMP LESS B/C MNT PRBS SF-12V2. 
Bind_Data$ADMALS42[Bind_Data$ADMALS42 < 0] <- NA
##4. ADPAIN42:  4WKS:PAIN LIMITS NORMAL WORK SF-12V2.
Bind_Data$ADPAIN42[Bind_Data$ADPAIN42 < 0] <- NA
##5. ADNRGY42:  4WKS: HAD A LOT OF ENERGY SF-12V2. 
Bind_Data$ADNRGY42[Bind_Data$ADNRGY42 < 0] <- NA 
##6. ADDOWN42:  4WKS: FELT DOWNHEARTED/DEPR SF-12V2.
Bind_Data$ADDOWN42[Bind_Data$ADDOWN42 < 0] <- NA 
##7. ADSOCA42: 4WKS: HLTH STOPPED SOC ACTIV SF-12V2. 
Bind_Data$ADSOCA42[Bind_Data$ADSOCA42 < 0] <- NA 

###Examine ranges and NAs###
summary(Bind_Data[,c("ADDAYA42","ADPWLM42","ADMALS42","ADPAIN42","ADNRGY42","ADDOWN42","ADSOCA42")])

#Step 2: Recode 5 item response in version2 to 2 item response of version1
...
##Step 3: Create variables for SF-6D
...
############################################################
## Priority Conditions recent <= 1 yr vs previous > year ##
############################################################
 
 
#Myocardial Infarction recent <= 1 yr vs previous > year
Bind_Data$MI_Recent_Previous_Classes <- factor(ifelse(Bind_Data$PCHeartAttackMI == 0 | Bind_Data$PCHeartAttackMI == 1, 0, 1), labels=c("MI Recent", "MI Previous")) 
 
#Ischemic Stroke recent <= 1 yr vs previous > year
Bind_Data$IS_Recent_Previous_Classes <- factor(ifelse(Bind_Data$PCStrokeTIAMiniStroke == 0 | Bind_Data$PCStrokeTIAMiniStroke == 1, 0, 1), labels=c("Stroke Recent", "Stroke Previous")) 
 
#Angina Pectoris recent <= 1 yr vs previous > year
Bind_Data$AP_Recent_Previous_Classes <- factor(ifelse(Bind_Data$PCAngina == 0 | Bind_Data$PCAngina == 1, 0, 1), labels=c("AP Recent", "AP Previous")) 
...
#Any CVD
Bind_Data<-Bind_Data %>% 
  mutate(anyCVD=ifelse(
    MyocardialInfarction_Classes=="Myocardial Infarction" |
      IschemicStroke_Classes=="Ischemic Stroke" |
      HeartFailure_Classes=="Heart Failure" |
      CardiacDysrhythmias_Classes=="Cardiac Dysrhythmias" |
      AnginaPectoris_Classes=="Angina Pectoris" |
      PeripheralArteryDisease_Classes=="Peripheral Artery Disease",1, 0))
Bind_Data$anyCVD<-as.factor(Bind_Data$anyCVD)

#Nonzero expenditure
Bind_Data<-Bind_Data %>% 
  mutate(nonzero=ifelse(AllPayers_Total==0,0,1))
table(Bind_Data$nonzero)

Bind_Data$nonzero<-relevel(factor(Bind_Data$nonzero), ref="0")
levels(Bind_Data$nonzero)
...
Bind_Data$AllPayers_Total_plus1 = ifelse(round(Bind_Data$AllPayers_Total) == 0, 1, round(Bind_Data$AllPayers_Total)) 
...
#Table 2 and Table 3: SF-6D Utility Scores and Total Expenditure by CVD Type using bootstrap
#1. Population analysis
Population_Analysis = mepsdsgn %>% 
  subset(((PANEL == 16 & Year == 2012) | (PANEL == 17 & Year == 2013) | (PANEL == 18 & Year == 2014) | (PANEL == 19 & Year == 2015) | (PANEL == 20 & Year == 2016)) & (AGE31X >= 18))

set.seed(123)
Population_Analysis_bootstrap<-as.svrepdesign(Population_Analysis,
                                                  type=c("bootstrap"),
                                                  fpc=NULL,
                                                  replicates=200,
                                                  compress=TRUE,
                                                  mse=FALSE)
#2. ICD code method
#(1) Subgroup analysis - MI
Subgroup_Analysis_MI = mepsdsgn %>% 
  subset(((PANEL == 16 & Year == 2012) | (PANEL == 17 & Year == 2013) | (PANEL == 18 & Year == 2014) | (PANEL == 19 & Year == 2015) | (PANEL == 20 & Year == 2016)) & ((MedicalCondition_1_Grouped == 1))& (AGE31X >= 18))

set.seed(123)
Subgroup_Analysis_MI_bootstrap<-as.svrepdesign(Subgroup_Analysis_MI,
                                         type=c("bootstrap"),
                                         fpc=NULL,
                                         replicates=200,
                                         compress=TRUE,
...
#(2) Subgroup analysis - IS
Subgroup_Analysis_IS = mepsdsgn %>% 
  subset(((PANEL == 16 & Year == 2012) | (PANEL == 17 & Year == 2013) | (PANEL == 18 & Year == 2014) | (PANEL == 19 & Year == 2015) | (PANEL == 20 & Year == 2016)) & ((MedicalCondition_2_Grouped == 1))& (AGE31X >= 18))

set.seed(123)
Subgroup_Analysis_IS_bootstrap<-as.svrepdesign(Subgroup_Analysis_IS,
                                         type=c("bootstrap"),
                                         replicates=200,
                                         compress=TRUE,
                                         mse=FALSE)

#(3) Subgroup analysis - HF
Subgroup_Analysis_HF = mepsdsgn %>% 
  subset(((PANEL == 16 & Year == 2012) | (PANEL == 17 & Year == 2013) | (PANEL == 18 & Year == 2014) | (PANEL == 19 & Year == 2015) | (PANEL == 20 & Year == 2016)) & ((MedicalCondition_3_Grouped == 1))& (AGE31X >= 18))

set.seed(123)
Subgroup_Analysis_HF_bootstrap<-as.svrepdesign(Subgroup_Analysis_HF,
                                     type=c("bootstrap"),
                                     replicates=200,
                                     compress=TRUE,
                                     mse=FALSE)

#(4) Subgroup analysis - CD
Subgroup_Analysis_CD = mepsdsgn %>% 
  subset(((PANEL == 16 & Year == 2012) | (PANEL == 17 & Year == 2013) | (PANEL == 18 & Year == 2014) | (PANEL == 19 & Year == 2015) | (PANEL == 20 & Year == 2016)) & ((MedicalCondition_4_Grouped == 1))& (AGE31X >= 18))

set.seed(123)
Subgroup_Analysis_CD_bootstrap<-as.svrepdesign(Subgroup_Analysis_CD,
                                         type=c("bootstrap"),
                                         replicates=200,
                                         compress=TRUE,
                                         mse=FALSE)

#(5) Subgroup analysis - AP
Subgroup_Analysis_AP = mepsdsgn %>% 
  subset(((PANEL == 16 & Year == 2012) | (PANEL == 17 & Year == 2013) | (PANEL == 18 & Year == 2014) | (PANEL == 19 & Year == 2015) | (PANEL == 20 & Year == 2016)) & ((MedicalCondition_5_Grouped == 1))& (AGE31X >= 18))

set.seed(123)
Subgroup_Analysis_AP_bootstrap<-as.svrepdesign(Subgroup_Analysis_AP,
                                         type=c("bootstrap"),
                                         replicates=200,
                                         compress=TRUE,
                                         mse=FALSE)

#(6) Subgroup analysis - PAD
Subgroup_Analysis_PAD = mepsdsgn %>% 
  subset(((PANEL == 16 & Year == 2012) | (PANEL == 17 & Year == 2013) | (PANEL == 18 & Year == 2014) | (PANEL == 19 & Year == 2015) | (PANEL == 20 & Year == 2016)) & ((MedicalCondition_6_Grouped == 1))& (AGE31X >= 18))

set.seed(123)
Subgroup_Analysis_PAD_bootstrap<-as.svrepdesign(Subgroup_Analysis_PAD,
                                         type=c("bootstrap"),
                                         replicates=200,
                                         compress=TRUE,
                                         mse=FALSE)
#(7) Subgroup analysis - no CVD
Subgroup_Analysis_NoCVD = mepsdsgn %>% 
  subset(
    ((PANEL == 16 & Year == 2012) | (PANEL == 17 & Year == 2013) | (PANEL == 18 & Year == 2014) | (PANEL == 19 & Year == 2015) | (PANEL == 20 & Year == 2016)) 
         & ((MedicalCondition_1_Grouped == 0) & 
              (MedicalCondition_2_Grouped == 0)& 
              (MedicalCondition_3_Grouped == 0)& 
              (MedicalCondition_4_Grouped == 0)& 
              (MedicalCondition_5_Grouped == 0)& 
              (MedicalCondition_6_Grouped == 0)) 
         & (AGE31X >= 18))

set.seed(123)
Subgroup_Analysis_NoCVD_bootstrap<-as.svrepdesign(Subgroup_Analysis_NoCVD,
                                                type=c("bootstrap"),
                                                replicates=200,
                                                compress=TRUE,
                                                mse=FALSE)
...
#3. Priority condition method
#(1) Subgroup Analysis - MI
Subgroup_Analysis_Priority_Recent_MI = mepsdsgn %>%
  subset((PCHeartAttackMI == 0 | PCHeartAttackMI == 1) 
         & (AGE31X >= 18)) 

set.seed(123)
Subgroup_Analysis_Priority_Recent_MI_bootstrap<-as.svrepdesign(Subgroup_Analysis_Priority_Recent_MI,
                                                               type=c("bootstrap"),
                                                               replicates=200,
                                                               compress=TRUE,
                                                               mse=FALSE)
...
Subgroup_Analysis_Priority_Previous_MI = mepsdsgn %>%
  subset(((PCHeartAttackMI == 2)) & 
           (AGE31X >= 18))

set.seed(123)
Subgroup_Analysis_Priority_Previous_MI_bootstrap<-as.svrepdesign(Subgroup_Analysis_Priority_Previous_MI,
                                                               type=c("bootstrap"),
                                                               replicates=200,
                                                               compress=TRUE,
                                                               mse=FALSE)
...
#(2) Subgroup Analysis - IS
Subgroup_Analysis_Priority_Recent_IS = mepsdsgn %>%
  subset((PCStrokeTIAMiniStroke == 0 | PCStrokeTIAMiniStroke == 1) 
         & (AGE31X >= 18)) 

set.seed(123)
Subgroup_Analysis_Priority_Recent_IS_bootstrap<-as.svrepdesign(Subgroup_Analysis_Priority_Recent_IS,
                                                               type=c("bootstrap"),
                                                               replicates=200,
                                                               compress=TRUE,
                                                               mse=FALSE)
...
Subgroup_Analysis_Priority_Previous_IS = mepsdsgn %>%
  subset(((PCStrokeTIAMiniStroke == 2)) & 
           (AGE31X >= 18))

set.seed(123)
Subgroup_Analysis_Priority_Previous_IS_bootstrap<-as.svrepdesign(Subgroup_Analysis_Priority_Previous_IS,
                                                               type=c("bootstrap"),
                                                               replicates=200,
                                                               compress=TRUE,
                                                               mse=FALSE)
#(3) Subgroup Analysis - AP
Subgroup_Analysis_Priority_Recent_AP = mepsdsgn %>%
  subset((PCAngina == 0 | PCAngina == 1) 
         & (AGE31X >= 18)) 

set.seed(123)
Subgroup_Analysis_Priority_Recent_AP_bootstrap<-as.svrepdesign(Subgroup_Analysis_Priority_Recent_AP,
                                                               type=c("bootstrap"),
                                                               replicates=200,
                                                               compress=TRUE,
                                                               mse=FALSE)

Subgroup_Analysis_Priority_Previous_AP = mepsdsgn %>%
  subset(((PCAngina == 2)) & 
           (AGE31X >= 18))

set.seed(123)
Subgroup_Analysis_Priority_Previous_AP_bootstrap<-as.svrepdesign(Subgroup_Analysis_Priority_Previous_AP,
                                                               type=c("bootstrap"),
                                                               replicates=200,
                                                               compress=TRUE,
                                                               mse=FALSE)
...
#Table 2: SF utility score by CVD Type using bootstrap
#1. Population analysis
svymean(~sf6d,
        design = Population_Analysis_bootstrap,
        na.rm = T) 

confint(svymean(~sf6d, 
                design = Population_Analysis_bootstrap,
                na.rm = T))
...
#2. ICD method - Subgroup Analysis
#(1) MI
svymean(~sf6d, 
        design = Subgroup_Analysis_MI_bootstrap,
        na.rm = T) 
...
#(2) IS
svymean(~sf6d, 
        design = Subgroup_Analysis_IS_bootstrap,
        na.rm = T) 
...
#(3) HF
svymean(~sf6d, 
        design = Subgroup_Analysis_HF_bootstrap,
        na.rm = T) 
...
#(4) CD
svymean(~sf6d, 
        design = Subgroup_Analysis_CD_bootstrap,
        na.rm = T) 
...
#(5) AP
svymean(~sf6d, 
        design = Subgroup_Analysis_AP_bootstrap,
        na.rm = T) 
...
#(6) PAD
svymean(~sf6d, 
        design = Subgroup_Analysis_PAD_bootstrap,
        na.rm = T) 
...
#(7) No CVD
svymean(~sf6d, 
        design = Subgroup_Analysis_NoCVD_bootstrap,
        na.rm = T) 
...
#3. Priority condition method - Subgroup Analysis
#(1-1) MI - Recent
svymean(~sf6d, 
        design = Subgroup_Analysis_Priority_Recent_MI_bootstrap,
        na.rm = T) 
...
#(1-2) MI - Previous
svymean(~sf6d, 
        design = Subgroup_Analysis_Priority_Previous_MI_bootstrap,
        na.rm = T) 
...
#(2-1) IS - Recent
svymean(~sf6d, 
        design = Subgroup_Analysis_Priority_Recent_IS_bootstrap,
        na.rm = T) 
...
#(2-2) IS - Previous
svymean(~sf6d, 
        design = Subgroup_Analysis_Priority_Previous_IS_bootstrap,
        na.rm = T) 
...
#(3-1) AP - Recent
svymean(~sf6d, 
        design = Subgroup_Analysis_Priority_Recent_AP_bootstrap,
        na.rm = T) 
...
#(3-2) AP - Previous
svymean(~sf6d, 
        design = Subgroup_Analysis_Priority_Previous_AP_bootstrap,
        na.rm = T) 
...
#4. Generate P values comparing recent <=1 yr vs previous > 1 yr diagnosis by parametric testing
 
#(1) Subgroup Analysis - MI
Subgroup_Analysis_Priority_MI = mepsdsgn %>%
  subset((PCHeartAttackMI >= 0) 
         & (AGE31X >= 18)) 
 
model1 <- svyglm(sf6d ~ MI_Recent_Previous_Classes, 
              design = Subgroup_Analysis_Priority_MI)
regTermTest(model1,"MI_Recent_Previous_Classes")
 
#(1) Subgroup Analysis - IS
Subgroup_Analysis_Priority_IS = mepsdsgn %>%
  subset((PCStrokeTIAMiniStroke >= 0) 
         & (AGE31X >= 18)) 
 
model1 <- svyglm(sf6d ~ IS_Recent_Previous_Classes, 
                 design = Subgroup_Analysis_Priority_IS)
regTermTest(model1,"IS_Recent_Previous_Classes")
 
#(1) Subgroup Analysis - AP
Subgroup_Analysis_Priority_AP = mepsdsgn %>%
  subset((PCAngina >= 0) 
         & (AGE31X >= 18)) 
 
model1 <- svyglm(sf6d ~ AP_Recent_Previous_Classes, 
                 design = Subgroup_Analysis_Priority_AP)
regTermTest(model1,"AP_Recent_Previous_Classes")

#Table 3: Total expenditure by CVD Type using bootstrap
#1. Population analysis
svymean(~AllPayers_Total, 
        design = Population_Analysis_bootstrap,
        na.rm = T) 

confint(svymean(~AllPayers_Total, 
                design = Population_Analysis_bootstrap,
                na.rm = T))

svyquantile(~AllPayers_Total, 
            design = Population_Analysis_bootstrap, 
            quantiles = c(.25, .5, .75), 
            na.rm = T)
...
#(1) Impact of Diagnoses and Patient Characteristics on Utility Values (SF-6D)
...
#(2) Impact of Diagnoses and Patient Characteristics on Total Annual Expenditures: Two-part model
##(1) Part 1: 
Population_LinearRegression_AllPayers_Total_twopart1<-svyglm(nonzero~MyocardialInfarction_Classes + 
                                                              IschemicStroke_Classes + 
                                                              HeartFailure_Classes + 
                                                              CardiacDysrhythmias_Classes + 
                                                              AnginaPectoris_Classes + 
                                                              PeripheralArteryDisease_Classes + 
                                                              Diabetes_Classes + 
                                                              AGE31X_Classes + 
                                                              SEX_Classes + 
                                                              RACETHX_Classes + 
...
summary(Population_LinearRegression_AllPayers_Total_twopart1)$coefficients
confint(Population_LinearRegression_AllPayers_Total_twopart1)

##Part 2: 
Population_Analysis_twopart_nonzero<-Population_Analysis %>% subset(nonzero=="1")

Population_LinearRegression_AllPayers_Total_twopart2<-svyglm(AllPayers_Total~MyocardialInfarction_Classes + 
                                                              IschemicStroke_Classes + 
                                                              HeartFailure_Classes + 
                                                              CardiacDysrhythmias_Classes + 
                                                              AnginaPectoris_Classes + 
                                                              ...
                                                              BMINDX53_Classes + 
                                                              Charlson_Classes,
                                                            design=Population_Analysis_twopart_nonzero,
                                                            family=Gamma(link="log"))

summary(Population_LinearRegression_AllPayers_Total_twopart2)
confint(Population_LinearRegression_AllPayers_Total_twopart2)
...
##Model 1: 
Population_LinearRegression_sf6d_1= svyglm(sf6d ~ MyocardialInfarction_Classes + IschemicStroke_Classes + HeartFailure_Classes + CardiacDysrhythmias_Classes + AnginaPectoris_Classes + PeripheralArteryDisease_Classes + Diabetes_Classes + AGE31X_Classes + SEX_Classes, design = Population_Analysis) #Model 1
summary(Population_LinearRegression_sf6d_1)
confint(Population_LinearRegression_sf6d_1)
plot(Population_LinearRegression_sf6d_1)

##Model 2:
Population_LinearRegression_sf6d_2 = svyglm(sf6d ~ MyocardialInfarction_Classes + IschemicStroke_Classes + HeartFailure_Classes + CardiacDysrhythmias_Classes + AnginaPectoris_Classes + PeripheralArteryDisease_Classes + Diabetes_Classes + AGE31X_Classes + SEX_Classes + RACETHX_Classes + INSURC_Classes + POVCAT_Classes + EDUCATION_Classes, design = Population_Analysis) #Model 2
summary(Population_LinearRegression_sf6d_2)
confint(Population_LinearRegression_sf6d_2)

##Model 3: 
Population_LinearRegression_sf6d_interaction = svyglm(sf6d ~ MyocardialInfarction_Classes +
                                                          IschemicStroke_Classes +
                                                          HeartFailure_Classes +
                                                          CardiacDysrhythmias_Classes +
                                                          AnginaPectoris_Classes +
                                                          ...
                                                          EDUCATION_Classes+
                                                          anyCVD+
                                                          anyCVD*AGE31X_Classes,
                                                        design = Population_Analysis)
summary(Population_LinearRegression_sf6d_interaction)
confint(Population_LinearRegression_sf6d_interaction)


#Appendix Table 3: Multivariable Generalized Gamma Regression of Total Annual Expenditures by CVD Type and Patient Characteristics - Two-part model
##Model 1: part 1
Population_LinearRegression_AllPayers_Total_twopart_m1p1<-svyglm(nonzero~MyocardialInfarction_Classes + 
                                                              IschemicStroke_Classes + 
                                                              HeartFailure_Classes + 
                                                              CardiacDysrhythmias_Classes + 
                                                              AnginaPectoris_Classes + 
                                                            ...
                                                            design=Population_Analysis,
                                                            family=binomial(link="logit"))
summary(Population_LinearRegression_AllPayers_Total_twopart_m1p1)
confint(Population_LinearRegression_AllPayers_Total_twopart_m1p1)


##Model1: part 2
Population_Analysis_twopart_nonzero<-Population_Analysis %>% subset(nonzero=="1")

Population_LinearRegression_AllPayers_Total_twopart_m1p2<-svyglm(AllPayers_Total~MyocardialInfarction_Classes + 
                                                              IschemicStroke_Classes + 
                                                              HeartFailure_Classes + 
                                                              CardiacDysrhythmias_Classes + 
                                                              AnginaPectoris_Classes + 
...
summary(Population_LinearRegression_AllPayers_Total_twopart_m1p2)
confint(Population_LinearRegression_AllPayers_Total_twopart_m1p2)


##Model 2: part 1
Population_LinearRegression_AllPayers_Total_twopart_m2p1<-svyglm(nonzero~MyocardialInfarction_Classes + 
                                                              IschemicStroke_Classes + 
                                                              HeartFailure_Classes + 
                                                              CardiacDysrhythmias_Classes + 
                                                              AnginaPectoris_Classes + 
                                                              ...
                                                              design=Population_Analysis,
                                                              family=binomial(link="logit"))
summary(Population_LinearRegression_AllPayers_Total_twopart_m2p1)
confint(Population_LinearRegression_AllPayers_Total_twopart_m2p1)

##Model 2: part 2
Population_LinearRegression_AllPayers_Total_twopart_m2p2<-svyglm(AllPayers_Total~MyocardialInfarction_Classes + 
                                                                  IschemicStroke_Classes + 
                                                                  HeartFailure_Classes + 
                                                                  CardiacDysrhythmias_Classes + 
                                                                  AnginaPectoris_Classes + 
...
summary(Population_LinearRegression_AllPayers_Total_twopart_m2p2)
confint(Population_LinearRegression_AllPayers_Total_twopart_m2p2)

##Model 3: part 1
Population_LinearRegression_AllPayers_Total_twopart_m3p1<-svyglm(nonzero~MyocardialInfarction_Classes + 
                                                                  IschemicStroke_Classes + 
                                                                  HeartFailure_Classes + 
                                                                  CardiacDysrhythmias_Classes + 
                                                                  AnginaPectoris_Classes + 
                                                                ...
                                                                design=Population_Analysis,
                                                                family=binomial(link="logit"))
summary(Population_LinearRegression_AllPayers_Total_twopart_m3p1)
confint(Population_LinearRegression_AllPayers_Total_twopart_m3p1)


##Model 3: part 2
Population_LinearRegression_AllPayers_Total_twopart_m3p2<-svyglm(AllPayers_Total~MyocardialInfarction_Classes + 
                                                                  IschemicStroke_Classes + 
                                                                  HeartFailure_Classes + 
                                                                  CardiacDysrhythmias_Classes + 
                                                                  AnginaPectoris_Classes + 
...
summary(Population_LinearRegression_AllPayers_Total_twopart_m3p2)
confint(Population_LinearRegression_AllPayers_Total_twopart_m3p2)
###############################################################
![image](https://user-images.githubusercontent.com/50415082/112463842-1e8ad780-8d39-11eb-8a3f-7a6d70d789f8.png)
